...
// Simple chat types matching Google Gen AI SDK (Types are removed for JS)
...
export default function AItest({
  ...
  const ai = useMemo(() => {
    try {
      // Typescript's `new GoogleGenAI({ apiKey })` is the same in JS
  ...
  async function sendMessage(message) {
    ...
    try {
      ...
      // Type is inferred for `h` from setHistory's initialization
      setHistory(h => [...h, { role: 'model', parts: [{ text: reply }] }]);
    } catch (err) {
      // The `err: any` type assertion is removed in JS
  ...
  // The type of `text` and return type are removed for JS
    ...
    <div style={styles.wrap}>
      <div style={styles.card}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
          {['請問最近有什麼國際要事發生嗎？', '請幫我想一段英文諧音梗笑話。', '查詢台北天氣。'].map((q) => (
...
// The type `Record<string, React.CSSProperties>` is removed in JS
const styles = {
